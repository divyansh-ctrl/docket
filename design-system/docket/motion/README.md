...
`docket_pipeline.py` animates the product thesis end to end: one request is
...
into a single route receipt, and a human — never Docket — approves.
...
- Stage names mirror the `stages` array in
  [`apps/dashboard/app/docket-dashboard.tsx`](../../../apps/dashboard/app/docket-dashboard.tsx).
...
```bash
manim -pql docket_pipeline.py MissionPipeline
```
...
```bash
manim -qk -t docket_pipeline.py MissionPipeline
```

Output lands in `media/videos/docket_pipeline/`. `-t` produces a `.mov` with an
alpha channel; convert to `.webm` for the browser if the clip is embedded:

```bash
ffmpeg -i media/videos/docket_pipeline/2160p60/MissionPipeline.mov \
